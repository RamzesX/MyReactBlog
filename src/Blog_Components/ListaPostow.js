...
class ListaPostow extends React.Component {
  ...
  changeContent(pageNumber) {
    fetch("https://ghost.techautority.tech/ghost/api/v3/content/posts?key=f854db3727ebf596c066859b88&limit=4&filter=tag:" + this.getTag(this.state.url) +"&page=" + (pageNumber + 1).toString())
    .then((res) => res.json())
    .then((json) => {
